...
struct gyroscopeDataView: View {
    ...
    var body: some View {
        VStack {
            Text("Gyroscope Data Acquisition")
                .font(.title)
                .foregroundStyle(Color.mint)
            
            TabView {
                List(motionManager.gyroscopeData, id: \.self) { data in
                    Text(data)
                }
            }
            .tabViewStyle(PageTabViewStyle(indexDisplayMode: .never))
            
            if let latestData = motionManager.gyroscopeData.last {
                Text(latestData)
                    .padding()
            } else {
                Text("No Data")
            ...
            VStack {
                if isRecordingInterval {
                    ...
                    Toggle(isOn: $isRecording) {
                        Text("Turn the toggle on to start automatic Gyroscope Data retrieval")
                            .padding()
