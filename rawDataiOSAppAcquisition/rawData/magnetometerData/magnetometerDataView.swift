...
struct magnetometerDataView: View {
    ...
    var body: some View {
        VStack {
            ...
            TabView {
                List(motionManager.magnetometerData, id: \.self) { data in
                    Text(data)
                }
            }
            .tabViewStyle(PageTabViewStyle(indexDisplayMode: .never))
            
            if let latestData = motionManager.magnetometerData.last {
                Text(latestData)
                    .padding()
            } else {
                Text("No Data")
            ...
            VStack {
                if isRecordingInterval {
                    ...
                    Toggle(isOn: $isRecording) {
                        Text("Turn the toggle on to start automatic Magnetometer Data retrieval")
                            .padding()
