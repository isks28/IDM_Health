...
class MagnetometerManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    let baseFolder: String = "MagnetometerData"
    ...
    @objc private func appDidEnterBackground() {
        ...
        if magnetometerManager.isMagnetometerActive {
    ...
    func startMagnetometerDataCollection(realTime: Bool, serverURL: URL) {
        ...
        if magnetometerManager.isMagnetometerAvailable {
            magnetometerManager.magnetometerUpdateInterval = 1.0 / currentSamplingRate
    ...
    func updateSamplingRate(rate: Double) {
        ...
        if isCollectingData {
            ...
            startMagnetometerDataCollection(realTime: recordingMode == "RealTime", serverURL: self.serverURL!)
        }
    }
    ...
    func scheduleDataCollection(startDate: Date, endDate: Date, serverURL: URL, baseFolder: String, completion: @escaping () -> Void) {
        ...
        if startDate > now {
            ...
            Timer.scheduledTimer(withTimeInterval: startInterval, repeats: false) { [weak self] _ in
                self?.startMagnetometerDataCollection(realTime: false, serverURL: serverURL)
